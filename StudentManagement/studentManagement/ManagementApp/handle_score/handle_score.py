...
from studentManagement.ManagementApp.models import *
...
# tạo ô,
def get_score_belong_type(type,id_student, list):
    #list data điểm tất cả học sinh để nó lấy lọc
    list_tmp = []  # list này dùng để lưu danh sách điểm theo loại
    print('gia tri list cell theo type',list_tmp)
    for i in list:
        if i[5] == id_student and i[2] == type:
            list_tmp.append(float(i[1]))

    # lấy ra được danh sách điểm theo kiểu 15m
    # print(list_tmp)
    # tạo id khởi tạo 1 ma trận
...
# lay diem mon hoc cua mon học lop trong nam hoc (def lấy điểm 1)
def get_score_student_in_class(id_year_hk, id_class, subject):
...
# tạo danh sách cell trên hệ thống
def create_data_list_cell(id_year,id_class,id_subject):

    list_cell = []

    # lấy full điểm toàn hs của lớp đó
    list_score = get_score_student_in_class(id_year_hk= id_year,id_class=id_class,subject= id_subject)
    # lấy danh sách id của hs trong lớp, năm học đó
...
# hàm xử lý nhập điểm của giáo viên
...
def handle_score():
    ...
    # dữ liệu json bên cell gửi vào trong request.json
    if request.method.__eq__('POST'):
        json_cell = request.json
        print("in json",json_cell)
        ...
        # lấy đc cel bị thay đổi
        cell = cell_maxtrix_score(id = id,list= l)
        # lấy ra ma trận cũ để so sánh cell => quyết định thay đổi
        print("CELL MOI ",cell)
        ...
        try:
            for i in  convert_chain_score_to_list(cell.list):
                ...
                score_st.id_school_year = id_year # gắn year sau
        ...
        print("sesion bên hàm này ", session['rule_subject'])
    return jsonify(session['rule_subject'])


# xử lý nhập điểm ( tùy vào môn mà nó có bao nhiêu cột để nhập khác nhau )
# hàm này lấy ra 1 diction các gia trị quy định

# hàm lấy cái luật lệ quy đinh mỗi môn học
...
def get_ratio_average_subject(id_subject):
    ...
    return ratio
# tính toán điểm trung bình ( mỗi khi thống kê điểm của từng học sinh) Xuất bảng điểm cuối năm
# ---------
